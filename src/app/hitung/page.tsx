...
async function drawShareCard(expenses: Expense[], total: number) {
  ...
  const brandUrl = "prediksi-bulanan.adityandar.my.id";
  context.fillStyle = "#64736b"; context.font = "600 25px Arial, sans-serif";
  const brandWidth = context.measureText(brandUrl).width + (brandIcon ? 80 : 0);
  const brandX = (width - brandWidth) / 2;
  if (brandIcon) context.drawImage(brandIcon, brandX, 1828, 60, 60);
  context.fillText(brandUrl, brandX + (brandIcon ? 80 : 0), 1866);
  return canvas;
}
...
export default function HitungPage() {
  ...
  function exportImage() { if (!expenses.length) return; setIsExporting(true); requestAnimationFrame(() => { void drawShareCard(expenses, total).then((canvas) => { if (canvas) { const link = document.createElement("a"); link.download = "prediksi-bulanan.png"; link.href = canvas.toDataURL("image/png"); link.click(); trackEvent("export_image", { item_count: expenses.length }); } }).finally(() => setIsExporting(false)); }); }
  ...
  return <main className="min-h-screen bg-[#faf9f6] pb-16"><header className="border-b border-[#e8e6e0] bg-[#faf9f6]"><Container className="flex h-16 items-center justify-between"><Link href="/" className="group flex items-center gap-2.5 font-semibold tracking-[-0.02em]"><Image src="/brand/logo-wordmark.png" alt="Prediksi Bulanan" width={220} height={48} priority className="h-8 w-auto sm:h-9" /></Link><Link href="/" className="text-sm font-medium text-[#667069] transition hover:text-[#287a5d]">← Beranda</Link></Container></header><Container className="max-w-3xl py-8 sm:py-12"><div className="mx-auto max-w-2xl">
